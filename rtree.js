...
var RStarTree = function RTree(M, m, p) {
    ...
    Node = function NodeFactory(parent) {
        ...
        split = function split() {
            ...
            // choose split axis
            splitAxis = (
              splitAxisScore(['left', 'right', 'bottom', 'top']) <
              splitAxisScore(['bottom', 'top', 'left', 'right']))
            ? { major: ['left', 'right'], minor: ['bottom', 'top'] }
            : { major: ['bottom', 'top'], minor: ['left', 'right'] };
...
OpenLayers.Strategy.RTree = OpenLayers.Class(OpenLayers.Strategy, {
    ...
    activate: function activate() {
        'use strict';
        var undefined,
        activated = OpenLayers.Strategy.prototype.activate.call(this);
        if (activated) {
            this.clearIndex();
            ...
            while (i--) {
                if (undefined === this.indexedListeners[i].source) {
                    this.indexedListeners[i].source = this.layer;
                }
    ...
    createIndex: function createIndex(event) {
        ...
        while (i--) {
            this.Tree.insert(event.features[i]);
        }
    },
